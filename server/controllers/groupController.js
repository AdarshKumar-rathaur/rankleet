...
exports.getGroupById = async (req, res) => {
    try {
        ...
        const group = await Group.findById(groupId).populate("members", "name leetcodeUsername stats").populate("createdBy", "_id");
        if (!group) {
            return res.status(404).json({ message: "Group not found" });
        }
...
exports.getLeaderboard = async (req, res) => {
    try {
        ...
        const group = await Group.findById(groupId).populate("members", "name leetcodeUsername stats");
        if (!group) {
            return res.status(404).json({ message: "Group not found" });
        }
